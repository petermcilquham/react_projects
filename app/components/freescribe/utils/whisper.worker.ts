...
async function load_model_callback(data: { status?: string; file?: any; progress?: any; loaded?: any; total?: any }) {
    const { status } = data
    if (status === 'progress') {
...
class GenerationTracker {
    ...
    chunkCallback(data: any) {
        ...
        this.processed_chunks = chunks.map((chunk: any, index: string) => {
    ...
    processChunk(chunk: { text: string; timestamp: any }, index: string) {
...
function createResultMessage(results: any, isDone: boolean, completedUntilTimestamp: number | undefined) {
    self.postMessage({
...
function createPartialResultMessage(result: { text: string; start: number | undefined; end: undefined }) {
    self.postMessage({
